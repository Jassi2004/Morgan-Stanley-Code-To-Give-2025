...
export default function ChatBot() {
  ...
  const handleSubmit = async () => {
    ...
    if (!students || students.length === 0) {
      setResponse([{ type: "system", message: "No student data available in context!" }]);
      return;
    }

    setIsLoading(true);
    try {
      const prompt = `Given the following student data: ${JSON.stringify(students)}. Answer the question: ${userInput}`;
